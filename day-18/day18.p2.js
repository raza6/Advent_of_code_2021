...
function solve() {
  ...
  for (let i = 0; i < fnumbers.length; i++) {
    for (let j = 0; j < fnumbers.length; j++) {
      if (i !== j) {
        fnumbers[i] = parseFarray(fbases[i]);
        fnumbers[j] = parseFarray(fbases[j]);
        let currentMagnitude = magnitude(add(fnumbers[i], fnumbers[j]));
  ...
  console.log('Result : ', maxMagnitude);
}
